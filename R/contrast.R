...
adjust_defaults <- function(model, t1 = NULL, t2 = NULL, t_mediator = NULL) {
  ...
  list(t1 = t1, t2 = t2, t_mediator = t_mediator)
}

direct_effect <- function(model, t1 = NULL, t2 = NULL, t_mediator = NULL) {
  t_prep <- adjust_defaults(model, t1, t2)
  t1 <- t_prep$t1
  t2 <- t_prep$t2
  ...
  bind_rows(result, .id = "mediator_setting") |>
    mutate(mediator_setting = names(t_mediator)[as.integer(mediator_setting)]) |>
    select(outcome, mediator_setting, contrast, direct_effect)
}
