...
In most microbiome mediation analysis studies, ismicrobiome composition is
...
```{r outcome_alteration, fig.width = 8, fig.height = 12}
...
list(
  ...
  ggplot() +
  geom_boxplot(
    aes(value, reorder(name, value), fill = treatment)
  ) +
  scale_x_log10() +
  facet_grid(~ source)
```
...
```{r direct_contrast, fig.width = 8, fig.height = 4}
...
ggplot(fdr, aes(direct_effect, fdr_hat)) +
  geom_label_repel(data = filter(fdr, keep), aes(label = outcome, fill = source), max.overlaps = 50, size = 3) +
  labs(x = "Direct Effect", y = "Estimated False Discovery Rate") +
...
```{r indirect_contrast, fig.width = 8, fig.height = 4}
