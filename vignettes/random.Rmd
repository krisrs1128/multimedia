...
```{r new_treat}
t_mediator <- data.frame(treatment = factor(c("Treatment", "Control", "Control", "Control")))
t_outcome <- t_mediator
t_outcome$treatment[1] <- "Control"
...
```{r contrast}
profile_control <- setup_profile(model, treatments(model)[rep(1, 4),, drop = FALSE], t_outcome)
