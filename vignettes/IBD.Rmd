...
direct_summary <- function(model, exper, t1 = 1, t2 = 3, N = 10) {
  direct_effect(model, exper, t1, t2) |>
    group_by(outcome) |>
    summarise(direct_effect = mean(direct_effect)) |>
    slice_max(abs(direct_effect), n = N) |>
    arrange(direct_effect) |>
    filter(direct_effect != 0)
}

indirect_summary <- function(model, exper, t1 = 1, t2 = 3, N = 10) {
  indirect_pathwise(model, exper, t1, t2) |>
    group_by(outcome, mediator) |>
    summarise(indirect_effect = mean(indirect_effect)) |>
    slice_max(abs(indirect_effect), n = N) |>
...
indirect_sorted
plot_mediators(indirect_sorted, exper, treatment = "Study.Group")
```
...
filter_outcomes <- inference$indirect |>
  ...
  slice_max(abs(indirect_effect), n = 60) |>
  pull(outcome)
...
filter_outcomes <- inference$direct |>
  ...
  slice_max(abs(direct_effect), n = 60) |>
  pull(outcome)
