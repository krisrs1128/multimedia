...
top direct and indirect effects in the object `overall_effects`.

```{r}
indirect_overall <- list(
  CD = indirect_overall(model, exper, 1, 3),
  UC = indirect_overall(model, exper, 2, 3)
)

top_direct <- rename(direct, effect = direct_effect)
top_indirect <- rename(bind_rows(indirect_overall), effect = indirect_effect)
top_effects <- list(direct = top_direct, indirect = top_indirect) |>
  bind_rows( .id = "type") |>
  group_by(outcome, type) |>
  summarise(effect = mean(abs(effect))) |>
  group_by(type) |>
  slice_max(abs(effect), n = 3)
```
...
coords <- data.frame(mds$points) |>
  ...
  bind_cols(exper@outcomes[, unique(top_effects$outcome)]) |>
  pivot_longer(top_effects$outcome, names_to = "outcome", values_to = "abundance") |>
  left_join(top_effects)

ggplot(coords) +
  geom_point(aes(X1, X2, col = Study.Group, size = abundance)) +
  scale_size(range = c(.1, 1.5), breaks = c(0, 4, 8)) +
  labs(
    x = glue("MDS1 [{eig(mds$eig, 1)}%]"),
    y = glue("MDS1 [{eig(mds$eig, 2)}%]"),
    size = "Metabolite Abundance",
    col = "Group"
  ) +
...
```{r lasso_mediators_plot, fig.width = 12, fig.height = 6}
p <- indirect_sorted %>%
  split(.$treatment) |>
  map(~ plot_mediators(., exper, treatment = "Study.Group", nrow = 2)) +
  labs(col = "Group")
