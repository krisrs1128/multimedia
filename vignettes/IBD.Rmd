...
mediation variables simultaneously. This allows us to analyze treatment $\arrow$
microbiome $\arrow$ metabolome and treatment $\arrow$ metabolome $\arrow$
microbiome paths. The methodology only allows us to analyze one kind of path at
a time; in any case, it's impossible to completely disambiguate the
directionality of the association from a single timepoint without prior
knowledge. That said, the relationships that are uncovered from either of these
directions can already suggest novel microbe-metabolite relationships.
...
ensures uniform data processing. Our specific study is from [Franzosa et al.'s](https://doi.org/10.1038/s41564-018-0306-4) study of intestinal bowel disease. We'll treat disease status as the
treatment, the microbiome as mediators, and metabolites as the outcome, in the
spirit of that study's discussion:
...
sense. For example, genus Copromorpha seems to have a negative relationship with
C18_eg_Clser_0930holae. Maybe that metabolite is produced by a competitor? Or
maybe the microbe eats the metabolite. In any case, IBD seems to decrease the
abundance of Copromorpha, which consequently increases the abundance of the
metabolite.

```{r lasso_mediators_plot, fig.width = 12, fig.height = 9}
...
samples <- list(
  ...
  mutate(name = factor(name, levels = direct$outcome))
...
```{r lasso_bootstrap, fig.width = 8.5, fig.height = 7}
fs <- list(direct = direct_effect, indirect = indirect_overall)
inference <- bootstrap(model, exper, fs, B = 1e3)
...
indirect <- indirect_pathwise(model, exper2)
indirect_sorted <- indirect |>
  group_by(outcome, mediator) |>
  summarise(indirect_effect = mean(indirect_effect)) |>
...
the analogous figure for the sparse regression.
