...
#sensitivity_curve <- sensitivity(fit, exper, n_rho = 4)
sensitivity_curve <- read_csv("https://drive.google.com/uc?export=download&id=1fEhg-aMRyfhAjgcJmsrxz2cmWCOdnM6C")
ggplot(sensitivity_curve, aes(rho, col = outcome, fill = outcome)) +
  geom_vline(xintercept = 0, linewidth = 0.5) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  geom_line(aes(y = indirect_effect)) +
  geom_ribbon(aes(
    y = indirect_effect, 
  ...
  labs(
    x = expression("Strength "~rho~"of Hypothetical Confounder"),
    y = "Overall Indirect Effect"
  )
```
...
tic(); rmarkdown::render("illustration.Rmd"); toc(); 9 seconds
tic(); rmarkdown::render("mindfulness.Rmd"); toc(); 246.947 seconds
tic(); rmarkdown::render("IBD.Rmd"); toc(); 78.152 seconds
tic(); rmarkdown::render("random.Rmd"); toc(); 44.5 seconds